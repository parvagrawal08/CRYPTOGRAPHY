...
def des_block(block, subkeys, decrypt=False):
    bits = bytes_to_bits(block)
    bits = permute(bits, IP)
    L, R = bits[:32], bits[32:]
    keys = subkeys[::-1] if decrypt else subkeys
    for i in range(16):
        newR = xor(L, feistel(R, keys[i]))
        L = R
        R = newR
    combined = R + L
...
def des_encrypt(plaintext, key):
    ...
    for i in range(0, len(data), 8):
        out += des_block(data[i:i+8], subkeys, decrypt=False)
    return out
...
def des_decrypt(ciphertext, key):
    ...
    for i in range(0, len(ciphertext), 8):
        out += des_block(ciphertext[i:i+8], subkeys, decrypt=True)
    return unpad(out).decode()

if __name__ == "__main__":
    mode = input("Mode (1 for encryption / 2 for decryption): ").strip().lower()
    key = input("Enter key (any text): ").encode()
    ...
    if mode == "1":
        ...
        hex_text = input("Enter hex ciphertext: ").strip()
